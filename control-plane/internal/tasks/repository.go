...
func (r *Repository) Create(ctx context.Context, task *Task) (*Task, error) {
	var created Task
	err := database.WithTenant(ctx, r.pool, task.TenantID, func(q database.Querier) error {
		row := q.QueryRow(ctx,
		...
		created.CreatedByName = r.fetchUserName(ctx, q, created.CreatedBy)
		return nil
	})
	if err != nil {
		return nil, err
...
func (r *Repository) fetchUserName(ctx context.Context, q database.Querier, userID uuid.UUID) string {
	var name string
	if err := q.QueryRow(ctx, `SELECT COALESCE(name, '') FROM users WHERE id = $1`, userID).Scan(&name); err != nil {
		return ""
	}
	return name
}
