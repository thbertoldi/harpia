...
<div class="flex h-full">
	<div class="flex-1 overflow-y-auto p-6 lg:p-8">
		<div class="mx-auto max-w-3xl">
			...
			<p class="font-body text-crown-ash mb-8">Review agent outputs that need your attention.</p>
			...
			{:else if feedbacks.length === 0}
				<div class="flex flex-col items-center justify-center py-20 text-center">
					...
					<p class="mt-1 font-body text-sm text-crown-ash">Nothing needs your review.</p>
				</div>
			{:else}
				<div class="space-y-3">
					{#each feedbacks as fb (fb.id)}
						<button
							...
							class="w-full rounded-lg border text-left transition-all {selectedId === fb.id
							...
							<div class="flex items-start gap-4 p-4">
								<div class="flex-1 min-w-0">
									<div class="flex items-center gap-2 mb-1">
										<span class="inline-flex items-center rounded-full bg-talon-gold/10 px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-talon-gold animate-pulse">
											Awaiting Your Review
										</span>
										{#if fb.createdAt}
											<span class="font-mono text-[10px] text-crown-ash-dark flex items-center gap-1">
												<Clock class="size-3" />
												{timeAgo(fb.createdAt)}
											</span>
										{/if}
									</div>
									<p class="font-body text-sm font-medium text-cream truncate">{fb.question}</p>
									<div class="mt-2 flex items-center gap-3">
										<span class="font-mono text-[10px] text-crown-ash-dark">Task: {shortId(fb.taskId)}</span>
										{#if fb.agentInstanceId}
											<span class="font-mono text-[10px] text-crown-ash-dark">Agent: {shortId(fb.agentInstanceId)}</span>
										{/if}
									</div>
								</div>
