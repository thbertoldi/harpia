...
<div class="flex flex-1">
  <!-- Chat area -->
  <div class="flex flex-1 flex-col min-h-0 {taskCreated ? 'max-w-[60%]' : ''} transition-all duration-500">
    {#if !taskCreated}
      <!-- Landing state -->
      <div class="flex flex-1 flex-col items-center justify-center px-4">
        <div class="mb-4 text-talon-gold">
          <svg class="size-12" viewBox="0 0 48 48" fill="none">
            <path d="M24 4L6 18v12l18 14 18-14V18L24 4z" stroke="currentColor" stroke-width="2" fill="none"/>
            <path d="M24 16l-6 4v6l6 4 6-4v-6l-6-4z" fill="currentColor" opacity="0.3"/>
          </svg>
        </div>
        ...
        <div>
          ...
          <div class="h-2 w-full rounded-full bg-muted overflow-hidden">
            ...
            {:else if task.status === TaskStatus.FAILED}
              <div class="h-full w-full rounded-full bg-red-500 transition-all duration-500"></div>
            {:else if task.status === TaskStatus.IN_PROGRESS}
              <div class="h-full w-3/4 rounded-full bg-talon-gold animate-pulse transition-all duration-500"></div>
            {:else if task.status === TaskStatus.PLANNING}
              <div class="h-full w-1/4 rounded-full bg-talon-gold animate-pulse transition-all duration-500"></div>
            {:else if task.status === TaskStatus.AWAITING_FEEDBACK}
              <div class="h-full w-1/2 rounded-full bg-yellow-500 transition-all duration-500"></div>
            {:else if task.status === TaskStatus.PENDING}
              <div class="h-full w-[10%] rounded-full bg-crown-ash transition-all duration-500"></div>
            {:else if task.status === TaskStatus.CANCELLED}
              <div class="h-full w-full rounded-full bg-crown-ash transition-all duration-500"></div>
            {/if}
          </div>
        </div>
