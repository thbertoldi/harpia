...
<div class="flex min-h-screen">
  <div class="fixed inset-y-0 z-40 flex">
    ...
    <!-- Hidden lateral nav -->
    <div
      class="h-full overflow-hidden transition-all duration-300 {navOpen ? 'w-64' : 'w-0'}"
    >
      <nav class="h-full w-64 border-r border-plumage bg-obsidian px-4 py-6">
        <div class="mb-8 flex items-center justify-between">
          ...
          <button onclick={toggleNav} class="rounded-md p-1 text-crown-ash hover:text-cream transition-colors">
            <X class="size-5" />
          </button>
        </div>
        ...
        <div class="mt-8 border-t border-plumage pt-6">
          <p class="font-mono text-[10px] uppercase tracking-widest text-crown-ash">Role</p>
          <p class="mt-1 font-body text-sm text-cream">Leader</p>
        </div>
      </nav>
    </div>
  </div>
  ...
  <!-- Main content -->
  <div class="flex flex-1 flex-col {navOpen ? 'ml-[calc(5px+16rem)]' : 'ml-[5px]'} transition-all duration-300">
    <!-- Header -->
    <header class="sticky top-0 z-30 border-b bg-background/80 backdrop-blur">
      <div class="flex h-14 items-center justify-between px-4 lg:px-6">
        <div class="flex items-center gap-3">
          <button
            onclick={toggleNav}
            class="rounded-md p-1.5 text-foreground hover:bg-muted transition-colors"
            aria-label="Toggle navigation"
          >
            <Menu class="size-5" />
          </button>
          <span class="font-heading text-lg font-bold tracking-tight text-foreground">
        ...
        <button
          onclick={toggleDark}
          class="rounded-md p-2 text-crown-ash hover:text-foreground transition-colors"
