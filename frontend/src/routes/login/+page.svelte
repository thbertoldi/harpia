<script lang="ts">
	...
	const personas = [
		{ role: 'Leader' as const, label: 'Leader', subtitle: 'Direct tasks', icon: Crown },
		{ role: 'Overseer' as const, label: 'Overseer', subtitle: 'Approve & feedback', icon: Eye },
		{ role: 'Engineer' as const, label: 'Platform Engineer', subtitle: 'Configure agents', icon: Wrench }
	];
</script>
...
<div class="flex min-h-screen items-center justify-center bg-obsidian">
	...
	<div class="w-full max-w-md space-y-8 p-8">
		<div class="text-center">
			<h1 class="text-6xl font-black text-talon-gold tracking-tight" style="font-family: 'Bodoni Moda', serif">
				Harpia
			</h1>
		...
		<div class="space-y-4">
			<button
				...
				title={zitadelReady ? '' : 'Run scripts/sync-oidc-config.sh after the zitadel-register-client Job completes'}
			...
			<form onsubmit={(e) => { e.preventDefault(); zitadelReady && handleLogin(); }} class="space-y-3">
				<input
					...
					disabled={!zitadelReady}
					class="w-full rounded-lg border border-plumage bg-obsidian-light px-4 py-3 font-body text-cream placeholder:text-crown-ash-dark focus:border-talon-gold focus:outline-none focus:ring-1 focus:ring-talon-gold transition-colors disabled:cursor-not-allowed disabled:opacity-50"
					style="font-family: 'DM Sans', sans-serif"
				/>
				<button
					type="submit"
			...
			<div class="relative pt-2">
				...
				<div class="relative flex justify-center text-xs">
					<span class="bg-obsidian px-2 font-mono text-crown-ash-dark uppercase tracking-wider" style="font-family: 'JetBrains Mono', monospace">
						dev login
					</span>
				</div>
			</div>
			...
			<div class="grid grid-cols-1 gap-2">
				{#each personas as persona}
					...
					>
						<persona.icon class="size-5 shrink-0 text-crown-ash transition-colors group-hover:text-talon-gold" />
						<div class="min-w-0 flex-1">
							<div class="font-body text-sm font-medium text-cream" style="font-family: 'DM Sans', sans-serif">
								{persona.label}
							</div>
							<div class="font-mono text-[10px] uppercase tracking-wider text-crown-ash-dark" style="font-family: 'JetBrains Mono', monospace">
								{persona.subtitle}
							</div>
						</div>
					</button>
