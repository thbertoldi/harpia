<script lang="ts">
  ...
  const config = $derived(statusConfig[status] ?? statusConfig[TaskStatus.UNSPECIFIED]);
  const isActive = $derived(pulsing && (status === TaskStatus.PLANNING || status === TaskStatus.IN_PROGRESS));
</script>
...
>
  {#if pulsing}
    <span class="relative flex h-2 w-2">
      <span class="{isActive ? 'animate-ping' : ''} absolute inline-flex h-full w-full rounded-full opacity-75 {config.dot}"></span>
