...
{:else if submitted}
	<div class="flex flex-col items-center justify-center py-16 transition-all duration-500 animate-in fade-in">
		<div class="flex h-16 w-16 items-center justify-center rounded-full bg-talon-gold/20">
		...
		<p class="mt-1 font-body text-sm text-crown-ash">
			{submittedDecision === 'Approve' ? 'Agent work approved.' : submittedDecision === 'Reject' ? 'Sent back to agent for retry.' : 'Modification feedback sent.'}
		</p>
	</div>
{:else if feedback}
	<div class="transition-all duration-300">
		...
		<div class="mb-6 rounded-lg border border-plumage bg-obsidian-light p-5">
			<p class="font-mono text-[10px] uppercase tracking-widest text-crown-ash mb-2">Agent Request</p>
			<p class="font-body text-base text-cream leading-relaxed">{feedback.question}</p>
			{#if feedback.createdAt}
				<p class="mt-2 font-mono text-xs text-crown-ash-dark">Waiting {timeAgo(feedback.createdAt)}</p>
			{/if}
		</div>
		...
		{#if feedback.status === 1}
			<div class="mb-6">
				<label for="feedback-comment" class="font-mono text-[10px] uppercase tracking-widest text-crown-ash block mb-2">Comment (optional)</label>
				<textarea
					id="feedback-comment"
					bind:value={comment}
